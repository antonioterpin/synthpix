...
def test_compare_legacy_vs_grain_performance(tmp_path, mock_mat_files):
    ...
    np.testing.assert_allclose(
        legacy_batch.flow_fields, grain_batch.flow_fields, 
        err_msg="Flow fields mismatch", rtol=1e-5
    )
    
    np.testing.assert_allclose(
        legacy_batch.images1, grain_batch.images1, 
        err_msg="Images1 mismatch", atol=1 
    )
    ...
    t_legacy = timeit.timeit(
        lambda: loop_system(lambda: legacy_episodic.get_batch(batch_size), legacy_episodic.reset, steps), 
        number=1
    )
    
    t_grain = timeit.timeit(
        lambda: loop_system(lambda: grain_adapter.get_batch(batch_size), grain_adapter.reset, steps), 
        number=1
    )
            ...
            if prev_c == curr_c:
                 ...
                 if curr_t != expected_t:
                     ...
                     if broken_count <= 5: # Print first few errors
                         print(f"   [Order Error] Index {i}: Chunk {prev_c} t={prev_t} -> t={curr_t} (Expected {expected_t})")
