...
def test_hdf5_group_error(tmp_path):
    ...
    ds = HDF5DataSource([str(tmp_path)]) # Changed to list for consistency with other tests
    ...
    with pytest.raises(ValueError, match="Expected Dataset but got"):
        _ = ds[0] # Changed from `ds[0]== 2` to `_ = ds[0]` to correctly trigger the load and error
