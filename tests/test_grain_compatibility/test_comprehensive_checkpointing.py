...
def get_valid_config(paths, source_type, include_images=False, episode_length=0):
    ...
    config = {
        ...
        "p_hide_img1": 0.0,
        "p_hide_img2": 0.0,
        "min_speed_x": 0.0, "max_speed_x": 0.0,
        "min_speed_y": 0.0, "max_speed_y": 0.0,
        "noise_uniform": 0.0, "noise_gaussian_mean": 0.0, "noise_gaussian_std": 0.0,
        "output_units": "pixels",
        "dt": 1.0,
        "resolution": 1.0,
...
def test_checkpoint_matrix(source_type, mode, include_images, tmp_path):
    ...
    # 3. Configuration
    config = get_valid_config(paths, source_type, include_images, 10 if mode == "episodic" else 0)
...
def test_legacy_scheduler_exclusion(mock_mat_files, tmp_path):
    ...
    with pytest.raises(ValueError, match="Sampler does not provide access to Grain iterator|does not expose Grain iterator"):
...
def test_edge_case_modified_file_list(mock_mat_files, tmp_path):
    ...
    with pytest.raises(ValueError, match="match dataloader sampler|DataSource in checkpoint does not match"):
...
def test_edge_case_missing_files(mock_mat_files, tmp_path):
    ...
    with pytest.raises((FileNotFoundError, ValueError), match=".*not found.*|.*Failed to load.*"):
...
def test_edge_case_config_drift(mock_mat_files, tmp_path):
    ...
    with pytest.raises(ValueError, match="Sampler in checkpoint does not match dataloader sampler"):
        make(new_config, use_grain_scheduler=True, load_from=checkpoint_dir)
