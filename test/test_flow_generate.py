...
def test_speed_img_gen(particles_number, image_shape):
    ...
    # The idea is that each device will generate a part of the image
    ...
    # 2. Send particles and keys to the devices
    sharding_keys = NamedSharding(mesh, PartitionSpec(shard_particles))
    ...
    # 3. Create the jit function
    img_gen_from_data_jit = jax.jit(
        lambda keys, particles: img_gen_from_data(keys, image_shape, particles)
    )

    def run_img_gen_jit():
        result = img_gen_from_data_jit(keys_sharded, particles_sharded)
        result.block_until_ready()

    # Warm up the function
    run_img_gen_jit()

    # 4. Measure the time of the jit function
    total_time_jit = timeit.repeat(
        stmt=run_img_gen_jit, number=NUMBER_OF_EXECUTIONS, repeat=REPETITIONS
    )
    average_time_jit = min(total_time_jit) / NUMBER_OF_EXECUTIONS

    # 5. Check if the time is less than the limit
    assert (
