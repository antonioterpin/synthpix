...
class EpisodicDataSource(grain.RandomAccessDataSource):
    ...
    def __init__(
        ...
        Args:
            source: The underlying FileDataSource (e.g. MATDataSource) that loads individual files.
            batch_size: Number of parallel episodes (must match the batch_size of the DataLoader).
        ...
        # We assume the source.file_list contains ALL valid files sorted/grouped by dir?
    ...
    def _build_interleaved_file_list(self) -> list[tuple[str, int, int]]:
        ...
        # Note: We drop remainder episodes that don't fit into a full batch set.
        # This is consistent with 'drop_remainder' batching logic often used in training.
        ...
        for i in range(num_chunks):
            ...
            for d, s in chunk_starts:
                ...
                chunk_episodes.append(self.dir2files[d][s : s + self.episode_length])
            ...
            for t in range(self.episode_length):
                for ep_idx in range(self.batch_size):
                    ...
                    # Store (path, chunk_id, timestep_in_episode) so we can attach metadata
