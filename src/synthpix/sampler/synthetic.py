...
class SyntheticImageSampler(Sampler):
    ...
    def __init__(
        ...
        img_offset: tuple[int | float, int | float] = (
            generation_specification.img_offset
        )
        dt: float = generation_specification.dt
        all_devices: list[Any] = jax.devices()
        if device_ids is not None:
            devices = [
                all_devices[id] for id in device_ids if id < len(all_devices)
            ]
        else:
            devices = all_devices
        ...
        if mask_images is not None and (mask_images.shape != image_shape):
            raise ValueError(
                f"Mask shape {mask_images.shape} does not match"
        ...
        if device_ids is not None and (len(devices) == 0):
        ...
        self.shard_fields = "fields"  # Name of the axis for the device mesh
        ...
        if batch_size % ndevices != 0:
            self.batch_size = (batch_size // ndevices + 1) * ndevices
        else:
            self.batch_size = batch_size
        if flow_fields_per_batch % ndevices != 0:
            self.flow_fields_per_batch = (
                flow_fields_per_batch // ndevices + 1
            ) * ndevices
        else:
        ...
        # batch size must be divisible by the number of devices.
        # fixes it if not (using next divisible number instead).
        if batch_size % ndevices != 0:
            logger.warning(
        ...
        # flow field per batch must be divisible by the number of devices.
        # fixes it if not (using next divisible number instead).
        if flow_fields_per_batch % ndevices != 0:
            logger.warning(
                "Unique Flow fields per batch was not divisible"
