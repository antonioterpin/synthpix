    ...
    def deposit_pixel(
        ...
        for dx, dy, weight in [
            ...
            new_image = jax.lax.cond(
                cond,
                lambda img, yi=yi, xi=xi, val=val, weight=weight:
                    img.at[yi, xi].add(
                    val * weight
                ),
                lambda img: img,
...
def input_check_apply_flow(
    ...
    if particle_positions.shape[1] == 2 and flow_field.shape[2] != 2:
        raise ValueError("Particle positions are in 2D, \
            but the flow field is in 3D.")

    if particle_positions.shape[1] == 3 and flow_field.shape[2] != 3:
        raise ValueError("Particle positions are in 3D, \
    ...
    if not isinstance(flow_field_res_x, int | float) or flow_field_res_x <= 0:
        raise ValueError("flow_field_res_x must be a positive scalar \
            (int or float)")
    if not isinstance(flow_field_res_y, int | float) or flow_field_res_y <= 0:
        raise ValueError("flow_field_res_y must be a positive scalar \
            (int or float)")
    if not isinstance(flow_field_res_z, int | float) or flow_field_res_z <= 0:
        raise ValueError("flow_field_res_z must be a positive scalar \
            (int or float)")
...
def apply_flow_to_particles(
    ...
    if particle_positions.shape[1] == 2:
        ...
        def _update_position_2d(
            ...
            u = bilinear_interpolate(
                flow_field[..., 0], x, y) * flow_field_res_x
            v = bilinear_interpolate(
                flow_field[..., 1], x, y) * flow_field_res_y
        ...
        def _update_position_3d(
            ...
            # and w to z.
            u = trilinear_interpolate(
                flow_field[..., 0], x, y, z) * flow_field_res_x
            v = trilinear_interpolate(
                flow_field[..., 1], x, y, z) * flow_field_res_y
            w = trilinear_interpolate(
                flow_field[..., 2], x, y, z) * flow_field_res_z
