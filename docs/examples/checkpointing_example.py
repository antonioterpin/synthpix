...
This script demonstrates how to save and restore the entire state
of a SynthPix pipeline using the high-level `synthpix.save_checkpoint`
and `load_from` APIs.
...
def main() -> None:
    ...
    Raises:
        RuntimeError: If the config file is not found at the expected location.
    """
    # Define paths
    ...
    if not file_list_path.exists():
        print(
            f"Warning: {file_list_path} does not exist. \
    ...
    # Continue generation
    # The next batch should be exactly what would have come next
    # in the original run
    # i.e., step 3 (0, 1, 2 were consumed/saved)
    batch = next(resumed_sampler)
    print(
        f"[Run 2] Resumed Batch (Step {saved_step + 1}): \
    ...
    print(
        "\n✅ Checkpointing demo completed successfully. \
            Bit-perfect reproducibility is guaranteed."
    )
