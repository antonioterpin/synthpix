...
- [Kinematic training (random flow generation)](#generating-images-from-random-kinematic-flow-fields)
...
## Generating images from random (kinematic) flow fields

For *kinematic training* (Manickathan, Mucignat & Lunati, *Meas. Sci. Technol.* 33, 124006, 2022) you do not need any flow files at all: ``SynthPix`` can generate smooth random displacement fields in-memory and render a fresh particle-image pair on top of each. Because the ground-truth displacement is known exactly and the renderer (particle diameter, density, noise, occlusion) is re-randomized every batch, this yields an effectively unlimited, exactly-labelled training set.

Each field is generated by the random flow generator (RFG):

1. draw each velocity component independently from ``U(-1, 1)`` over the image grid;
2. low-pass filter each component with a Gaussian of width ``sigma`` (px) to give a finite correlation length;
3. scale the field by the sampled factor ``a`` according to ``scale_mode``.

``scale_mode`` controls how ``a`` is interpreted:

- ``"peak"`` (default, paper-faithful): each field is normalized so its **peak displacement magnitude equals ``a`` pixels**. The follow-up paper (Manickathan, Mucignat & Lunati, *Exp. Fluids* 64, 161, 2023, Table 1) parameterizes this exact dataset by "Maximum displacement, max(ds_ref) (px): 0–16". Gaussian filtering attenuates the noise amplitude by a strongly ``sigma``-dependent factor (~1/``sigma``), so peak normalization is what keeps ``a`` meaningful in pixels across the whole ``sigma`` range.
- ``"linear"``: apply ``a`` as a raw multiplier, ``ds_ref = a · G_sigma · ξ`` (the literal form of the equation). The realized displacement is then strongly ``sigma``-dependent — with the bare ``[0, 16]`` range and ``sigma`` up to 100 px most fields collapse to ~0.1 px, so this mode usually needs a re-calibrated range.

Per field, ``sigma`` and ``a`` are sampled uniformly from the configured ranges. Generation is deterministic in the record index, so the pipeline checkpoints and replays exactly.

To enable it, set ``scheduler_class: "kinematic"`` (no ``file_list``) and keep ``include_images: false`` — the image pair is generated, not read. The only kinematic-specific keys are:

```yaml
scheduler_class: "kinematic"        # Generate flows instead of reading files
num_examples: 18278                 # Size of the (virtual) flow-field pool; larger = more variety
filter_sigma_range: [5.0, 100.0]    # Gaussian filter width sigma (px), per field
scale_mode: "peak"                  # "peak": normalize to a target peak displacement; "linear": raw a·G_sigma·xi
scale_factor_range: [0.0, 16.0]     # Peak displacement target max(ds_ref) a (px), per field
```

All the usual image- and flow-generation parameters still apply (and re-randomize per batch as augmentation). A ready-to-use preset lives at [`config/kinematic.yaml`](../config/kinematic.yaml); with ``loop: true`` and ``episode_length: 0`` the sampler iterates indefinitely, so supervised training can run for any number of batches:

```python
sampler = synthpix.make("config/kinematic.yaml")

for batch in sampler:
    imgs1 = batch.images1     # shape (B, H, W) — rendered
    imgs2 = batch.images2     # shape (B, H, W) — rendered
    flows = batch.flow_fields # shape (B, H, W, 2) — exact ground truth
```

### References

The LIMA series (Empa, Swiss Federal Laboratories for Materials Science and Technology):

- Manickathan, L., Mucignat, C., & Lunati, I. (2022). "Kinematic training of convolutional neural networks for particle image velocimetry." *Measurement Science and Technology*, 33, 124006. <https://doi.org/10.1088/1361-6501/ac8fae> — *introduces the kinematic-training strategy and the random-flow generator (`ds_ref = a·G_σ·ξ`) implemented here.*
- Manickathan, L., Mucignat, C., & Lunati, I. (2023). "A lightweight neural network designed for fluid velocimetry." *Experiments in Fluids*, 64, 161. <https://doi.org/10.1007/s00348-023-03695-8> — *parameterizes the same kinematic dataset by "Maximum displacement, max(ds_ref) (px)" (Table 1), the basis for the default `scale_mode: "peak"`.*
- Mucignat, C., Zdybał, K., & Lunati, I. (2025). "Improving the performance of a lightweight convolutional neural network for particle image velocimetry through hyper-parameter and padding optimization." *Physics of Fluids*, 37, 105112. <https://doi.org/10.1063/5.0283779> — *latest in the series (LIMA estimator hyper-parameter and padding optimization).*
