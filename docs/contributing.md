...
- a useful commit history; and
- a rigorous testing pipeline.

For this, to contribute to the project, we suggest to read the following sections and understand our development workflow (if you believe we can improve the process, please let us know via issues!).

To start, we recommend checking [our additional installation instructions](./installing.md), which contains options useful when developing the `SynthPix` code.
